...
<form action="/pl/updateGlobalCode.do" method="post" name="bbsNttRegistForm" id="bbsNttRegistForm" onsubmit="return validateBoardRegist(this)">
<input type="hidden" id="areaNm" name="areaNm" value="${areaNm}" />
					...
					<div class="col-sm-5">
						<div class="input-group">						
							...
							<c:forEach var="globalCode" items="${globalCodeList}" varStatus="status">
								<option value="${globalCode.areaNm}"<c:if test="${globalCodeData.upperNm eq globalCode.areaNm}"> selected="selected"</c:if>>${globalCode.areaNm}</option>
							</c:forEach>
							</select>
						</div>				
					</div>						
				...
				<div class="form-group">
					...
					<div class="col-sm-5">
						<div class="input-group">
							...
							<input type="text" class="form-control" data-mask="" placeholder="" id="newAreaNm" name="newAreaNm" value="${globalCodeData.areaNm}"/>
						</div>
					</div>
				</div>
	...
	<div style="padding-top:50px; padding-bottom:10px; text-align:center">
		<button type="submit" id="registBtn" name="registBtn" class="btn btn-blue" style="width:80px">수정</button>
