...
<form name="boardNttListForm" id="boardNttListForm" method="get" action="/pl/mngConsList.do">
...
<div align="right" style="margin-bottom: 10px">
  				<select title="검색목록 선택" id="searchCondition" name="searchCondition" style="height:31px; vertical-align:bottom;">
					<option value="title" <c:if test="${boardNttVO.searchCondition eq 'title'}">selected="selected"</c:if>>문의유형</option>
					<option value="writer" <c:if test="${boardNttVO.searchCondition eq 'writer'}">selected="selected"</c:if>>성명</option>
				</select>
	...
	<table class="table table-bordered responsive">
		<thead>
			<tr height="40px">
				...
				<th style="text-align:center; vertical-align:middle">성명</th>							
		...
		<tbody>
			...
			<c:forEach var="result" items="${resultList}" varStatus="status">
				<tr>
					...
					<td align="center"><a href="/pl/mngConsView.do?boardNo=${result.boardNo}&contentNo=${result.contentNo}"><c:out value="${result.title}"/></td>
					<td align="center"><c:out value="${result.writer}"/></td>
					<td align="center"><c:out value="${result.field1}"/></td>
					<td align="center"><c:out value="${result.field2}"/></td>
			...
			<c:if test="${empty resultList}">
				<tr>
					<td align="center" colspan="6">검색된 내용이 없습니다.</td>
				</tr>
			</c:if>
		</tbody>
	</table>	
