...
<script type="text/javaScript" language="javascript" defer="defer">
	...
	function goPage(pageNo) {
		...
		document.boardNttListForm.action = "<c:url value='/pl/mngGolfClubList.do'/>";
	   	document.boardNttListForm.submit();
	}
	function fn_addView() {
	   	document.boardNttListForm.action = "<c:url value='/pl/addGolfClubView.do'/>";
...
<ol class="breadcrumb bc-3" >
	...
	<li>게시판 관리</li>
	<li class="active"><a href="/pl/mngGolfClubList.do?boardNo=${boardNo }"><strong>${boardKor }</strong></a></li>
</ol>
...
<form name="boardNttListForm" id="boardNttListForm" method="get" action="/pl/mngGolfClubList.do">
<input type="hidden" id="boardNo" name="boardNo" value="golf"/>
<input type="hidden" id="pageNo" name="pageNo" value="${pageNo }" />

<div align="left" style="margin-bottom: 10px">  				
	<a href="/pl/globalCodeList.do"><button type="button" class="btn btn-red">국가 관리</button></a>
</div>
	...
	<table class="table table-bordered responsive">
		<thead>
			<tr height="40px">
				...
				<th style="text-align:center; vertical-align:middle">대륙</th>							
				<th style="text-align:center; vertical-align:middle">국가</th>							
				<th style="text-align:center; vertical-align:middle">지역명</th>							
				<th style="text-align:center; vertical-align:middle">클럽명(한글)</th>							
				<th style="text-align:center; vertical-align:middle">클럽명</th>				
				<th style="text-align:center; vertical-align:middle;">골프 스토리</th>				
			</tr>
		</thead>
		<tbody>
			...
			<c:forEach var="result" items="${resultList}" varStatus="status">
				<tr>
					...
					<td align="center"><a href="/pl/mngGolfClubView.do?boardNo=${result.boardNo}&contentNo=${result.contentNo}"><c:out value="${result.field1}"/></a></td>
					<td align="center"><a href="/pl/mngGolfClubView.do?boardNo=${result.boardNo}&contentNo=${result.contentNo}"><c:out value="${result.field2}"/></a></td>
					<td align="center"><a href="/pl/mngGolfClubView.do?boardNo=${result.boardNo}&contentNo=${result.contentNo}"><c:out value="${result.field3}"/></a></td>
					<td align="center"><a href="/pl/mngGolfClubView.do?boardNo=${result.boardNo}&contentNo=${result.contentNo}"><c:out value="${result.title}"/></a></td>
					<td align="center"><a href="/pl/mngGolfClubView.do?boardNo=${result.boardNo}&contentNo=${result.contentNo}"><c:out value="${result.field4}"/></a></td>
					<td align="center">
					<c:if test="${result.linkUrl1 ne null && result.linkUrl1 ne ''}">
					<a href="<c:out value="${result.linkUrl1}"/>" target="_blank">바로가기</a>
					</c:if>
					</td>								
				</tr>
			...
			<c:if test="${empty resultList}">
				<tr>
					<td align="center" colspan="7">검색된 내용이 없습니다.</td>
				</tr>
			</c:if>
		</tbody>
	</table>	
